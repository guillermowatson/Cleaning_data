...
The script then do the following steps in an automated fashion:
...
3. By using the "procesaFiles" function, both the training and test data gets processed:
	* Loads the observations, activity and subjects file
	* Update the column names of the observations file using the features file
	* Eliminate the unwanted columns (keep only the mean and std info)
	* Add the columns regarding subjectsID and activityID to the observations file (cbinds activity and subjects files)
	* Join the previous file with the activity labels by activityID (using merge function)
